import {
  ...
  import { getCurrentPool, initSdk, txVersion } from './raydiumConfig'
  import BN from 'bn.js'
  import { isValidCpmm } from './raydiumConfig'
  import { PublicKey } from '@solana/web3.js'
  

 //npx ts-node swapBaseOut.ts
  ...
  export const swapBaseOut = async () => {
    ...
    // outputMint will be set from the fetched pool info to avoid mismatches
    ...
    // Set output mint to a mint that exists in the pool to avoid mismatch
    const outputMint = new PublicKey(poolInfo.mintB.address)
    // Means want to buy 1 unit with 9 decimals (works for SOL); logic below clamps to reserves anyway
